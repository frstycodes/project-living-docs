...
- **The painting is the one asset you base64-embed** (the museum host 403s without
  a custom header, which a browser `<img>` can't send). Everything else — avatars,
  icons — stays a live URL or inline SVG.
...
- `prefers-reduced-motion` kills every animation, transition, confetti, and sound.
  `shell.css`/`shell.js` already handle this. There is **no mute toggle** —
  reduced-motion is the only sound switch.
- `--ink-3` on `--page` clears 4.5:1.
- Set text with `textContent`. Parse and host-allowlist every URL before it
  becomes an `href`.

## Hard don'ts

- Never inject a model-generated string with `innerHTML`.
